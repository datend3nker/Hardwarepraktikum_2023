...
void teslaMode()
{
    float left = measureDistance(US1_PIN);
    float middle = measureDistance(US2_PIN);
    float right = measureDistance(US3_PIN);
    float border = 0.3;
    if (left > border && middle > border && right > border)
    {
        // hier vorwährts fahren
        
        drive(false, 300, 500);
    }
    else if ((left < border && middle > border) && right > border)
    {
        // Hindernis stark links
        turn(false, 200, 500);
    }
    else if ((left > border && middle > border) && right < border)
    {
        // hindernis stak rechts
        turn(true, 200, 500);
    }
    else if ((left < border && middle < border) && right > border)
    {
    ...
    {
        // hindernis vorne komplett
        turn(false, 300, 500);
    }
    else if ((left > border && middle < border) && right > border)
    {
        // hindernis vorne mitte
        turn(false, 300, 500);
    } 
    else 
    {
